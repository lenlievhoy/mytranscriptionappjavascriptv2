...
document.getElementById('go-to-btn').addEventListener('click', function() {
    const timeCodeInput = document.getElementById('go-to-input').value;
    goToTimeCode(timeCodeInput);
});
...
// Function to go to specified time code
function goToTimeCode(timeCode) {
    const timeComponents = timeCode.split(':');
    ...
    const media = document.querySelector('audio, video');
    media.currentTime = totalSeconds;
    media.play();
}
